 ...
 *  It takes 4 characters(6Bit算一個Character) "per 3 bytes of data"(每三個Bytes), plus potentially a bit of padding at the end.
	...
	Base32 = 0-9 + no a, i, o, l
	Base32 = 0-31 = 5 bits = Even+Odd+Even+Odd+Even
	Base64 = A-Z + a-z + 0-9 + "+/" 
	Base64 = 0-63 = 6 bits = 3 Bytes = 24 bits(4 times 6 bits) = 6 bits + prepadding 2 bits(00) = (to make 3 bytes become 4 bytes)
	E.g., 01001000 01100101 01101100 3 bytes
		  010010 00 0110 0101 01  101100 3 bytes
	      010010 000110  010101   101100 4 characters
	      XX010010 XX00010 XX010101 XX101100 padding to a byte, where X = 0
	        18d       6d      21d      45d
	        'S'       'G'     'V'      't' 
 ...
 * 
 * SPRITES: CSS sprites were a solution to the problem of multiple HTTP requests to download multiple images. 
 * BASE64:  Data URIs allow you to embed images directly into your CSS files, solving the same problem in a much more elegant and maintainable way. 
...
public class Base64 {
    ...
    // zero in the end to make it 3X bytes
    ...
    public static String encode(String string) {
        ...
        String encoded = "";
        // 1. Convert String to bytearray(binary format)
        byte[] stringArray;
        try {
        ...
        //2.  determine how many padding bytes to add to the output
        int paddingCount = (3 - (stringArray.length % 3)) % 3;
        // add any necessary padding to the input
        stringArray = zeroPad(stringArray.length + paddingCount, stringArray);
        
        
        //3. process 3 bytes at a time, churning out 4 output bytes
        // worry about CRLF insertions later
        for (int i = 0; i < stringArray.length; i += 3) {
        	// aggregate 3 byte (11111111 - 8 bits)
            int j = ((stringArray[i] & 0xff) << 16) +
                ((stringArray[i + 1] & 0xff) << 8) + 
                (stringArray[i + 2] & 0xff);
            // churning out 4 bytes = 6 bit a set to encode
            // 4. 64 Encode here!
            // (j >> 12) 6 bit base & 3f(111111 - 6 bits)
     ...
     * */
    //  CRLF line terminators (which Windows uses to terminate lines) "\r\n"
    // insert "\r\n" every 76 byte
